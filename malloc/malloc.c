...
#else
#define assert(expr)                                                           \
    ((expr) ? ((void) 0) : __malloc_assert(#expr, __FILE__, __LINE__, __func__))
...
static struct malloc_par mp_ = {.top_pad        = DEFAULT_TOP_PAD,
...
static void do_check_malloc_state(mstate av) {
    ...
    for (i = 0; i < NFASTBINS; ++i) {
        ...
        if (av == &main_arena && i > max_fast_bin) assert(p == 0);
            ...
            if (mm != MAP_FAILED) {
                ...
                if (MALLOC_ALIGNMENT == 2 * SIZE_SZ) {
                    /* For glibc, chunk2mem increases the address by 2*SIZE_SZ
                       and
                       MALLOC_ALIGN_MASK is 2*SIZE_SZ-1.  Each mmap'ed area is
                       page
        ...
        if ((long) (MINSIZE + nb - old_size) > 0 &&
            ...
            set_head(old_top,
                     (((char *) old_heap + old_heap->size) - (char *) old_top) |
                         PREV_INUSE);
        } else if ((heap = new_heap(nb + (MINSIZE + sizeof(*heap)),
                                    mp_.top_pad))) {
    ...
    { /* Request enough space for nb + pad + overhead */
        ...
        if (brk != (char *) (MORECORE_FAILURE)) {
            ...
            /*
               ...
               in
               address space, so sbrk cannot extend to give contiguous space,
               but
            ...
            if (contiguous(av)) size = ALIGN_UP(size + old_size, pagesize);
            ...
            if ((unsigned long) (size) > (unsigned long) (nb)) {
                char *mbrk =
                    (char *) (MMAP(0, size, PROT_READ | PROT_WRITE, 0));
             ...
             */
                ...
                if (contiguous(av)) {
                    ...
                    /*
                       ...
                       space
                       is contiguous with first sbrk. This is a safe assumption
                       unless
                       program is multithreaded but doesn't use locks and a
                       foreign sbrk
                ...
                if (snd_brk != (char *) (MORECORE_FAILURE)) {
                    ...
                    /*
                       ...
                       space
                       we don't own. These fenceposts are artificial chunks that
                       are
                       marked as inuse and are in any case too small to use.  We
                       need
                    ...
                    if (old_size != 0) {
                        ...
                        /*
                           ...
                           overwrite
                           old_top when old_size was previously MINSIZE.  This
                           is
...
static void internal_function munmap_chunk(mchunkptr p) {
    ...
    if (DUMPED_MAIN_ARENA_CHUNK(p)) return;
...
static mchunkptr internal_function mremap_chunk(mchunkptr p, size_t new_size) {
    ...
    if (size + offset == new_size) return p;
...
static void *_int_memalign(mstate av, size_t alignment, size_t bytes) {
    ...
    { /*
        ...
        if ((unsigned long) (brk - (char *) (p)) < MINSIZE) brk += alignment;
...
static int mtrim(mstate av, size_t pad) {
    ...
    if (arena_is_corrupt(av)) return 0;
...
#if MALLOC_DEBUG > 1
        if (i > 0) dump_heap(heap_for_ptr(top(ar_ptr)));
#endif
        system_b += mi.arena;
...
int __libc_mallopt(int param_number, int value) {
    ...
    case M_ARENA_TEST:
        if (value > 0) do_set_arena_test(value);
        break;
...
#define nsizes (sizeof(sizes) / sizeof(sizes[ 0 ]))
  ...
  <unsorted from=\"%zu\" to=\"%zu\" total=\"%zu\" count=\"%zu\"/>\n",
        ...
        if (ar_ptr != &main_arena) {
            ...
            fprintf(fp, "<aspace type=\"total\" size=\"%zu\"/>\n"
...
weak_alias(__malloc_info, malloc_info)
    ...
    strong_alias(__libc_calloc,
                 __calloc) weak_alias(__libc_calloc,
                                      calloc) strong_alias(__libc_free, __cfree)
        weak_alias(__libc_free,
                   cfree) strong_alias(__libc_free,
